...
**Trạng thái (2026-06-17):** **Phase 9 HOÀN THÀNH** — NICE: **Google OAuth + Admin Dashboard** (chi tiết + khái niệm ôn tập: [DEV-LOG.md](DEV-LOG.md)). **OAuth:** Google Identity Services token flow (PIVOT từ Passport — D60); FE lấy ID token → `POST /auth/google` → BE verify bằng `google-auth-library` (chữ ký + audience) → tìm-hoặc-tạo user (`password_hash=null`, link theo email, `email_verified=true`) → trả `{user,token}` cùng shape `login()` nên FE tái dùng `AuthContext.login`. Không migration. Env `GOOGLE_CLIENT_ID`(BE)=`VITE_GOOGLE_CLIENT_ID`(FE) cùng 1 Client ID. **Dashboard:** `GET /api/admin/dashboard` adminOnly gộp 4 khối — KPI (doanh thu đơn Delivered, tổng đơn, đơn cần xử lý, tổng user), doanh thu/tháng (`$queryRaw date_trunc`, BigInt→number), top sách (`groupBy book_title` snapshot), đơn theo trạng thái (`groupBy status`); FE `AdminDashboardPage` (trang index `/admin`) vẽ bằng **Recharts** (D61). **207 unit test BE xanh**, FE build+lint xanh, verify browser (login render nút Google, dashboard 5 SVG chart + KPI đúng). **Trước đó:** Phase 8 Wishlist+Review+Recommend (D57–D59); Phase 7 Voucher (D54–D56); Phase 6 Email Resend (D50–D53); Phase 1–5 CORE end-to-end.
