...
**Trạng thái (2026-06-15):** **Phase 7 HOÀN THÀNH** — NICE: **Voucher + VoucherUsage** (chi tiết + khái niệm ôn tập: [DEV-LOG.md](DEV-LOG.md)). Mã giảm giá 2 loại (% có trần + số tiền cố định) trừ vào subtotal (`total = subtotal − discount + shipping`); `lib/voucher.ts` `calcDiscount` thuần (clamp ≤ subtotal). `modules/voucher` `validateVoucher` dùng chung cho `POST /vouchers/preview` (hiển thị) lẫn `createOrder` (chốt) — server tự lấy subtotal từ giỏ DB (D40/D56). Tích hợp transaction: createOrder snapshot `voucher_code`+`discount_amount`+`voucher_id`, tăng `used_count` compare-and-set (D45-style, D55) + log `VoucherUsage`; cancelOrder hoàn lượt + xóa usage. Admin CRUD voucher (chặn xóa mã đã có đơn — tắt thay vì xóa, D56). FE: ô mã + preview ở CheckoutPage (dòng giảm + cập nhật tổng), AdminVouchersPage. **177 unit test BE xanh**, build+lint FE xanh, verify browser (áp WELCOME10 → giảm 22k, tổng 240k→218k) + curl preview. **Review vòng 8** sửa per_user_limit race-safe (đếm trong tx sau khi updateMany giữ khóa dòng voucher — đúng cả limit=1) + expire_at cuối-ngày-local. **Trước đó:** Phase 6 Email service (Resend, D50–D53); Phase 1–5 CORE end-to-end (Auth→Catalog→Cart→Checkout→Order→Payment COD+VNPay).
