...
**Trạng thái (2026-06-17):** **Phase 8 HOÀN THÀNH** — NICE: **Wishlist + Review + Recommend** (chi tiết + khái niệm ôn tập: [DEV-LOG.md](DEV-LOG.md)). **Wishlist:** toggle theo `@@unique(user_id,book_id)`, cần đăng nhập, endpoint `/ids` cho FE tô tim (D57). **Review:** verified-purchase (chỉ ai có Order Delivered chứa sách — đếm lúc `upsertReview`, 403 nếu chưa mua), 1 review/sách dùng `upsert` để sửa, full CRUD của mình; rating **denormalized** `avg_rating`+`review_count` trên Book, recompute (`aggregate`+`update`) TRONG transaction mỗi lần review đổi (D58). **Recommend:** `GET /books/:slug/related` cùng tác giả/thể loại, loại self, limit 6 (D59). FE: `WishlistButton` tự chứa + `Stars` ở thẻ sách, `ReviewsSection` (list+form sửa/xóa) + `RelatedBooks` ở trang chi tiết, WishlistPage. **196 unit test BE xanh**, build+lint FE xanh, verify browser (trang chi tiết đủ 4 khối, recommend đúng) + curl wishlist. **Lưu ý nhánh:** Phase 2–7 từng nằm ở `feat/phase-2-3-catalog-cart`, đã fast-forward merge vào `main`; nay làm thẳng trên `main`. **Trước đó:** Phase 7 Voucher (D54–D56); Phase 6 Email (Resend, D50–D53); Phase 1–5 CORE end-to-end.
