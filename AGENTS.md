...
Preview: `.Codex/launch.json` đã khai báo 2 server tên `backend` / `frontend`. Đổi schema Prisma xong PHẢI chạy `npx prisma generate` (client nằm ở `backend/src/generated/`, gitignored, tự generate lại khi `npm install` nhờ postinstall).
